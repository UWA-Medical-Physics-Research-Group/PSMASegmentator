...
from nnunetpsma.configuration import default_num_processes
from nnunetpsma.inference.data_iterators import PreprocessAdapterFromNpy, preprocessing_iterator_fromfiles, \
    preprocessing_iterator_fromnpy
from nnunetpsma.inference.export_prediction import export_prediction_from_logits, \
    convert_predicted_logits_to_segmentation_with_correct_shape
from nnunetpsma.inference.sliding_window_prediction import compute_gaussian, \
    compute_steps_for_sliding_window
from nnunetpsma.utilities.file_path_utilities import get_output_folder, check_workers_alive_and_busy
...
from nnunetpsma.utilities.json_export import recursive_fix_for_json_export
from nnunetpsma.utilities.label_handling.label_handling import determine_num_input_channels
from nnunetpsma.utilities.plans_handling.plans_handler import PlansManager, ConfigurationManager
from nnunetpsma.utilities.utils import create_lists_from_splitted_dataset_folder
...
class nnUNetPredictor(object):
    ...
    def initialize_from_trained_model_folder(self, model_training_output_dir: str,
        ...
        trainer_class = recursive_find_python_class(join(nnunetpsma.__path__[0], "training", "nnUNetTrainer"),
                                                    trainer_name, 'nnunetpsma.training.nnUNetTrainer')
        if trainer_class is None:
            raise RuntimeError(f'Unable to locate trainer class {trainer_name} in nnunetpsma.training.nnUNetTrainer. '
...
def predict_entry_point_modelfolder():
    ...
    parser.add_argument('-device', type=str, default='cuda', required=False,
                        help="Use this to set the device the inference should run with. Available options are 'cuda' "
                             ...
                             "Use CUDA_VISIBLE_DEVICES=X nnunetpsma_predict [...] instead!")
...
def predict_entry_point():
    ...
    parser.add_argument('-num_parts', type=int, required=False, default=1,
                        help='Number of separate nnunetpsma_predict call that you will be making. Default: 1 (= this one '
                             'call predicts everything)')
    parser.add_argument('-part_id', type=int, required=False, default=0,
                        help='If multiple nnunetpsma_predict exist, which one is this? IDs start with 0 can end with '
                             'num_parts - 1. So when you submit 5 nnunetpsma_predict calls you need to set -num_parts '
    ...
    assert args.part_id < args.num_parts, 'Do you even read the documentation? See nnunetpsma_predict -h.'
...
if __name__ == '__main__':
    ...
    from nnunetpsma.paths import nnUNet_results, nnUNet_raw
    ...
    from nnunetpsma.imageio.simpleitk_reader_writer import SimpleITKIO
