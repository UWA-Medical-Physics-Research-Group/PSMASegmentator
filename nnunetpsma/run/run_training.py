...
from nnunetpsma.run.load_pretrained_weights import load_pretrained_weights
...
def get_trainer_from_args(dataset_name_or_id: Union[int, str],
    ...
    nnunet_trainer = recursive_find_python_class(join(nnunetpsma.__path__[0], "training", "nnUNetTrainer"),
    ...
    if nnunet_trainer is None:
        raise RuntimeError(f'Could not find requested nnunet trainer {trainer_name} in '
                           f'nnunetpsma.training.nnUNetTrainer ('
                           f'{join(nnunetpsma.__path__[0], "training", "nnUNetTrainer")}). If it is located somewhere '
...
def run_training_entry():
    ...
    parser.add_argument('-device', type=str, default='cuda', required=False,
                    help="Use this to set the device the training should run with. Available options are 'cuda' "
                         ...
                         "Use CUDA_VISIBLE_DEVICES=X nnunetpsma_train [...] instead!")
