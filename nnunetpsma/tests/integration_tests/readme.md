...
Set your pwd to be the nnunet repo folder (the one where the `nnunetpsma` folder and the `setup.py` are located!)

Now generate the 4 dummy datasets (ids 996, 997, 998, 999) from dataset 4. This will crash if you don't have Dataset004!
```commandline
bash nnunetpsma/tests/integration_tests/prepare_integration_tests.sh 
```

Now you can run the integration test for each of the datasets:
```commandline
bash nnunetpsma/tests/integration_tests/run_integration_test.sh DATSET_ID
```
...
Also run 
```commandline
bash nnunetpsma/tests/integration_tests/run_integration_test_trainingOnly_DDP.sh DATSET_ID
```
...
```commandline
python nnunetpsma/tests/integration_tests/cleanup_integration_test.py
```