...
from nnunetpsma.evaluation.evaluate_predictions import region_or_label_to_mask, compute_metrics_on_folder, \
    load_summary_json, label_or_region_to_key
from nnunetpsma.imageio.base_reader_writer import BaseReaderWriter
from nnunetpsma.paths import nnUNet_raw
from nnunetpsma.utilities.file_path_utilities import folds_tuple_to_string
    ...
    if plans_file_or_dict is None:
        ...
        if not isfile(expected_plans_file):
            raise RuntimeError(f"Expected plans file missing: {expected_plans_file}. The plans files should have been "
                               f"created while running nnunetpsma_predict. Sadge.")
    ...
    if plans_file_or_dict is None:
        ...
        if not isfile(expected_plans_file):
            raise RuntimeError(f"Expected plans file missing: {expected_plans_file}. The plans file should have been "
                               f"created while running nnunetpsma_predict. Sadge. If the folder you want to apply "
    ...
    if dataset_json_file_or_dict is None:
        ...
        if not isfile(expected_dataset_json_file):
            raise RuntimeError(
                ...
                f"copied while running nnunetpsma_predict/nnunetpsma_ensemble. Sadge.")
        dataset_json_file_or_dict = load_json(expected_dataset_json_file)
