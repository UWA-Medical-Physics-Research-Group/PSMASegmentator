...
from nnunetpsma.ensembling.ensemble import ensemble_crossvalidations
from nnunetpsma.evaluation.accumulate_cv_results import accumulate_cv_results
from nnunetpsma.evaluation.evaluate_predictions import compute_metrics_on_folder, load_summary_json
from nnunetpsma.paths import nnUNet_preprocessed, nnUNet_raw, nnUNet_results
from nnunetpsma.postprocessing.remove_connected_components import determine_postprocessing
from nnunetpsma.utilities.file_path_utilities import maybe_convert_to_dataset_name, get_output_folder, \
...
def generate_inference_command(dataset_name_or_id: Union[int, str], configuration_name: str,
    ...
    predict_command = f'nnunetpsma_predict -d {dataset_name_or_id} -i {input_folder} -o {output_folder} -f {fold_str} ' \
...
def print_inference_instructions(inference_info_dict: dict, instructions_file: str = None):
    ...
    if is_ensemble:
        ...
        _print_and_maybe_write_to_file(f"nnunetpsma_ensemble -i {output_folder_str} -o {output_ensemble} -np {default_num_processes}")

    _print_and_maybe_write_to_file("\n***Once inference is completed, run postprocessing like this:***\n")
    _print_and_maybe_write_to_file(f"nnunetpsma_apply_postprocessing -i OUTPUT_FOLDER -o OUTPUT_FOLDER_PP "
